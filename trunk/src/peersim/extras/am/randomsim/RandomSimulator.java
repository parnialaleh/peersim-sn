...
package peersim.extras.am.randomsim;
...
/**
 * This class is the main class for the Random Simulator, which
 * extends the range simulator by modifying how ranges are
 * selected. A range is a collection of values <em>S</em> to be 
 * assigned to a variable <em>v</em>. The Range Simulator invokes
 * the standard Peersim simulator by selecting one of the possible
 * values. If multiples ranges <em>S1, S2, ..., Sn</em> are specified, 
 * the standard Peersim simulator is invoked with a random tuple in 
 * <em>S1 * S2 * ... * Sn</em>.  
 * <p>
 * The number of experiments MUST be specified. If multiple experiments 
 * are required, each of them will be executed with a random tuple in 
 * <em>S1 * S2 * ... * Sn</em>.
 * <p>  
 ...
 */
public class RandomSimulator implements ProcessHandler
{
 ...
 * @config
 */
public static final String PAR_REXPERIMENTS = "simulation.experiments.random";
...
{
	RandomSimulator r = new RandomSimulator(args);
	r.run();
}
...
public RandomSimulator(String[] args)
{
...
private void parseRanges()
{
	...
	experiments = Configuration.getInt(PAR_REXPERIMENTS);
...
private void doExperiments(String[] args)
{
	...
	String[] rv = new String[pars.length];
	
	for (int i=0; i < experiments; i++) {

		// Configure the argument string array
		for (int j = 0; j < pars.length; j++) {
			int r = CommonState.r.nextInt(values[j].length);
			rv[j] = values[j][r];
			list.set(startpar + j, pars[j] + "=" + rv[j]);
		}
