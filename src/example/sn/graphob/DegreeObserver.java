...
{
	private final static String PAR_PID = "linkable";
	...
	public boolean execute()
	{
		...
		for (int i = 0; i < Network.size(); i++){
			System.out.println(CommonState.getTime() + " " + name + ": InDegree " + entry[i].inDegree + " SnInDegree " + entry[i].snInDegree + " " + ((float)entry[i].inDegree / (float)entry[i].snInDegree));
		}
